...
class Base_Agent(object):
    ...
    def get_action_size(self):
        ...
        if self.action_types == "DISCRETE": return 108
    ...
    def get_trials(self):
        ...
        if self.environment_title in ["AntMaze", "FetchReach", "Hopper", "Walker2d", "CartPole", "cart"]: return 100
        return 100
    ...
    def discrete_to_box(self, dis_action):
        row = dis_action // 18
        column = (dis_action % 18) // 3
        quarter = 0
        ...
        return np.array((row, column, quarter), dtype=int)

    def conduct_action(self, action):
        """Conducts an action in the environment"""
        self.environment.render()
        action = self.discrete_to_box(action)
        self.next_state, self.reward, self.done, _ = self.environment.step(action)

        if self.done is True:
            self.environment.render()
            self.environment.show_result()
